...
void CreateDatabase(string DbName)     //创建数据库
{
    db_info NewDatabase;
...
void ShowColumn(string TableName)     //显示当前数据库指定表的各列的名字
{
...
void UseDatabase(string DbName)        //指定当前操作数据库
{
    vector<db_info>::iterator it;
    int flag=0;
    for(it=dblist.begin();it<dblist.end();it++)
...
void CreateTable(string filename,vector<string> colname,vector<string> coltype)   //创建表，并加入到当前数据库中
{
    table_info NewTable;
