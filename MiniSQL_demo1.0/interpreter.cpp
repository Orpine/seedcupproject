...
bool interpreter::InitCommand(CString &cmd)       //初始化SQL命令
{
	int i,j,k,l;
	CString stack;
	stack.Empty();
...
bool interpreter::CreateTable(CString TableName,CString TableInfo)
{
	...
	for(k=0;;k++)
	{
		...
		if(AttName == "primary")
		{
			...
			for(i=0;i<k;i++)
				if(tb.info[i].attribute_name == tb.primary_key)
				{
					tb.info[i].unique = true;
					tb.info[i].has_index = true;
				}
			break;
		}
		if(k >= 32)
		...
		tb.info[k].attribute_name = AttName;
		s = p;
...
bool interpreter::print1(CString Table_name , CString attr)
{
	...
	//	cout << endl;
		...
		for(int b=num - 1;b>=0;b--)
		{
			for(a=0;a<Table_info.attribute_count;a++)
			{
				if(Table_info.info[a].type == "char")
	...
	//	cout << endl;
		...
		for(int b=num-1;b>=0;b--)
		{
			for(a=0;a<attrCount;a++)
			{
				if(Table_info.info[at[a]].type == "char")
