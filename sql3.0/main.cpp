   ...
   // while(1)
    {
		...
		switch(fk_word)
		{
			...
			case DROP:
				//删除表
				if(sk_word==TABLE)
					tf = DropTable(fileName);

				else if(sk_word==DATABASE)
					tf = DropDatabase(fileName);
				else
...
bool DropDatabase(string DbName)            //删除指定的数据库
{
    vector<db_info>::iterator it;
    int flag=0;
    for(it=dblist.begin();it<dblist.end();it++)
...
bool DropDbTable(string DbName,string TableName)       //删除指定数据库的某个表
{
    vector<db_info>::iterator it;
    int flag=0;
    for(it=dblist.begin();it<dblist.end();it++)
    ...
    for(p=(*db_p).tlist.begin();p<(*db_p).tlist.end();p++)
        {
            if((*p).tablename==TableName)
            {
                ...
                else
                {
                    ...
                    if(ColSnum.size()< ((*p).colist.size()-1)){
                        for( i = 0; i < (*p).colist.size()-1; i++){
